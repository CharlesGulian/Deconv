...
for testImage1 in goodImgs:
    for testImage2 in goodImgs:
        ...
        biasSub = True
        if biasSub:
            testImage1_biasSub = testImage1.replace('.fits','_biasSub.fits')
            testImage2_biasSub = testImage2.replace('.fits','_biasSub.fits')
            
            fits_tools.subtractBias(testImage1,new_image_file=testImage1_biasSub,bias=1000.0)
            fits_tools.subtractBias(testImage2,new_image_file=testImage2_biasSub,bias=1000.0)
            
            testImage1 = testImage1_biasSub
            testImage2 = testImage2_biasSub
        ...
        for k in range(len(varParamRange)):
            ...
            #temp = fig1.config_dict['CHECKIMAGE_NAME'].replace('Results/CheckImages','Figures/Jul20/'+varParam).replace('.fits','_{}{}.fits'.format(varParam,varParamRange[k]))
            ...
            # Change check image name
            #temp = fig2.config_dict['CHECKIMAGE_NAME'].replace('Results/CheckImages','Figures/Jul20/'+varParam).replace('.fits','_{}{}.fits'.format(varParam,varParamRange[k]))
            ...
            rads = img1data.get_data('FLUX_RADIUS')
            image1,image2 = fits_tools.getPixels(testImage1),fits_tools.getPixels(testImage2)
            flux1,flux2 = np.zeros(np.shape(x)),np.zeros(np.shape(x))
    
            for i in range(len(x)):
                flux1[i] = fits_tools.computeObjectFlux(x[i],y[i],rads[i],image1)
                flux2[i] = fits_tools.computeObjectFlux(x[i],y[i],rads[i],image2)
            ...
            SAVE = True
            if SAVE:
                plt.savefig(os.path.join(curr_dir,'Figures','Jul20',varParam,'flux1_flux2_hist_{}{}.png'.format(varParam,varParamRange[k])))
                plt.close()
            else:
                plt.show()
            ...
            SAVE = True
            if SAVE:
                plt.savefig(os.path.join(curr_dir,'Figures','Jul20',varParam,'fluxRatio_hist_{}{}.png'.format(varParam,varParamRange[k])))
                plt.close()
            else:
                plt.show()
            ...
            SAVE = True
            if SAVE:
                plt.savefig(os.path.join(curr_dir,'Figures','Jul20',varParam,'fluxRatio_map_{}{}.png'.format(varParam,varParamRange[k])))
                plt.close()
            else:
                plt.show()
            ...
            SAVE = True
            if SAVE:
                plt.savefig(os.path.join(curr_dir,'Figures','Jul20',varParam,'fluxRatioBins_hist_{}{}.png'.format(varParam,varParamRange[k])))
                plt.close()
            else:
                plt.show()
